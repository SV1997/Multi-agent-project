...
def set_supervisor_agent(checkpointer):
    ...
    classifier_llm = init_chat_model(model="groq:openai/gpt-oss-120b", temperature=0, streaming=False, reasoning_effort="low").with_structured_output(DomainClassification, method="function_calling", strict=True).with_config(tags=["classification-only"])
